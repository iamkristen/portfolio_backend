...
async function getContactInfo(req, res) {
    try {
        ...
        if (!result) {
            return res.json({ success: false, error: "Contact information not found." });
        }
...
async function updateContactInfo(req, res) {
    try {
        ...
        const data = req.body;
        const updatedData = await contactMeModel.findByIdAndUpdate(id, data, { new: true });

        if (!updatedData) {
            return res.json({ success: false, error: "Failed to update contact information." });
        }
...
async function deleteContactInfo(req, res) {
    try {
        ...
        if (!deletedData) {
            return res.json({ success: false, error: "Failed to delete contact information." });
        }

        res.json({ success: true, data: "Contact information deleted successfully." });
    } catch (error) {
